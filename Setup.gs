...
function createTransactionSheet(ss) {
  ...
  // 시트가 이미 존재하고 데이터가 있으면 스킵 (기존 데이터 보존)
  if (sheet) {
    const lastRow = sheet.getLastRow();
    if (lastRow > 1) {
      Logger.log('거래내역통합 시트가 이미 존재하고 데이터가 있어 스킵합니다.');
      return;
    }
  }
...
function createRulesSheet(ss) {
  ...
  // 시트가 이미 존재하고 데이터가 있으면 스킵 (기존 규칙 보존)
  if (sheet) {
    const lastRow = sheet.getLastRow();
    if (lastRow > 1) {
      Logger.log('분류규칙 시트가 이미 존재하고 데이터가 있어 스킵합니다.');
      return;
    }
  }
  ...
  const rules = [
    ...
    [8, '고정비', '보안', '에스케이쉴더스', '사업', '보안 서비스'],
    [7, '고정비', '세무', '세무사|세무기장|세무대행|윤현', '사업', '세무사 수수료'],
    [7, '고정비', '배상보험', '배상책임|의료배상', '사업', '의료 배상책임보험'],
    [7, '고정비', '4대보험', '4대사회보험|4대보험|국민연금|건강보험공단', '사업', '대내 보험'],
    [7, '고정비', '국세', '국세|지방세|세금납부', '사업', '세금'],

    // ========== 변동비 ==========
    [10, '변동비', '카드수수료', '카드수수료|PG수수료|결제수수료', '사업', '카드 결제 수수료'],
    [9, '변동비', '환불', '환불|취소|반환', '사업', '환불 처리'],
    [9, '변동비', '청소', '그린캔크리|에이치엘디|한라|청소|당신의집사', '사업', '청소 용역'],
    [8, '변동비', '식비', '이두부야|나주관|술까락|쿠팡이츠|VIC마켓|세븐일레븐|GS25|CU|이마트|파리바게뜨|뚜레쥬르|BBQ|스타벅스|카페|매머드익스프레스|케이에프씨|KFC|목동명가떡집|부타덴|떡볶이|우아한형제|버거|빵|일출옥|죽|김가네|과자|순대|국밥|규동|지에스25|두부', '사업', '직원 식비'],
    [8, '변동비', '약재', '행림원외탕전|옥천당|한약|약재|생약|한방|경희약업|광동|탕전|서울의과학연구소', '사업', '한약재 및 탕전'],
    [7, '변동비', '비품', '침|부항|뜸|거즈|알코올|소독|의료용|다이소|문구|사무용품|복사|쿠팡|네이버페이|밀리|리디|스마일페이|옥션|G마켓|케이엠솔루션|삼성전자|카카오', '사업', '의료/사무용 비품'],
    [6, '변동비', '교통비', '카카오택시|택시|우버|타다|영진주유소|주유|GS칼텍스|SK에너지', '확인필요', '업무용만 인정'],
    [6, '변동비', '주차장', '시설관리공단', '사업', '주차 및 통행료'],
    ...
    // ========== 개인지출 ==========
    [3, '개인지출', '식비', '', '개인', '개인 식사'],
    [3, '개인지출', '생활비', '', '개인', '개인 생활용품'],
    [3, '개인지출', '백화점', '', '확인필요', '사업/개인 구분 필요'],
    [3, '개인지출', '차량', '', '개인', '개인 차량 (사업용은 변동비)'],
    [3, '개인지출', '기타개인', '교습소|학원|CGV|메가박스', '개인', '애완동물, 학원 등'],
    [2, '개인지출', '의류비', '', '개인', '개인 의류'],
...
function createMonthlyReportSheet(ss) {
  ...
  if (sheet) {
    const lastRow = sheet.getLastRow();
    if (lastRow > 1) {
      Logger.log('월간보고서 시트가 이미 존재하고 데이터가 있어 스킵합니다.');
      return;
    }
  }
...
function createAccountantSheet(ss) {
  ...
  if (sheet) {
    const lastRow = sheet.getLastRow();
    if (lastRow > 1) {
      Logger.log('세무사전달 시트가 이미 존재하고 데이터가 있어 스킵합니다.');
      return;
    }
  }
...
function createTaxInvoiceSheet(ss) {
  ...
  if (sheet) {
    const lastRow = sheet.getLastRow();
    if (lastRow > 1) {
      Logger.log('세금계산서내역 시트가 이미 존재하고 데이터가 있어 스킵합니다.');
      return;
    }
  }
